...
TABLES = {
    "users": "账号（客户与管理员）",
    "products": "面料产品目录（281 条，**id 就是产品号文本，如 'P0001'**，含品类/颜色/价格/库存）",
    "orders": "订单（业务核心）",
    "refunds": "退款工单（挂在订单号上）",
...
11. **products.id 就是产品号本身（TEXT，'P0001' 这种），orders.product_id 同格式**：
   - 表里**没有** product_no / product_code / product_id 列 —— 别去"找"它们；
   - 关联对话消息里的产品号（P0075）直接 `JOIN products p ON p.id = 'P0075'` 等值匹配即可；
   - **不要**用 `'P' || lpad(id, 4, '0')` 之类去"生成"产品号：id 已经是完整产品号，
     再拼一次只会拼出 'PP0001' 这种永远匹配不上的东西（多轮评测实测翻车点）。
"""
