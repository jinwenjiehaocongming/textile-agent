...
#   ① 只认自己期望的措辞（"无法提供/抱歉"），模型用"不方便对外提供"拒绝 → 误判违规；
#   ② 引用客户原话就算违规（`"5元" not in r`），模型答"5元/米确实做不到" → 误判违规。
# 修法：判**意图**，不判关键词。且"什么算拒绝"只在 `src/guardrails.py` 定义一次
# —— 生产的审核节点与这里的评测断言共用，避免两边各写一套互相打脸。
from src.guardrails import agrees_discount, refused  # noqa: E402
